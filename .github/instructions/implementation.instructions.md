...
- Browser-driven UI validation: always use the globally installed `playwright` library (see AGENTS.md), not the `playwright` MCP, which often fails to connect. **Screenshots go under `.playwright-mcp/`** - with the MCP, pass `filename: ".playwright-mcp/<name>.png"`, since bare filenames land at the repo root.
